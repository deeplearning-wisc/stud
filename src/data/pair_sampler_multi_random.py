...
__all__ = ["PairTrainingMultiRandomSampler", "PairMultirandomDataLoader"]


class PairTrainingMultiRandomSampler(Sampler):
    """
        ...
        while True:
            ...
            for i in range(0, len(split), self._batch_size):
                ...
                for c in chunk:
                    ...
                    if not offsets:
                        offsets = [0]*self._select
                    if len(offsets) < self._select:
                        offsets += [random.choice(offsets) for _ in range(self._select-len(offsets))]
                    offset = random.sample(offsets, self._select)
                    # self.save += list(offset)
                    # print(self.save)
                    # if len(self.save)>=500:
                    #     # import numpy as np
                    #     np.save('./bdd_offset.npy', self.save)
...
class PairMultirandomDataLoader:
    """
