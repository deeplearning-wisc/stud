...
from .performer import PerformerSelfAttention
...
class MsViT(Backbone):
    ...
    def __init__(self, arch, img_size=512, in_chans=3,
                 ...
                 out_features=None, freeze_at=0, #detectron2
                 **args):
        super().__init__()
        ...
        if attn_type == "performer":
            ...
            self.feature_redraw_interval = args['redraw_interval']
        ...
        self._out_feature_strides['layer1'] = down_strides[0]
        self._out_feature_channels['layer1'] = self.layer_cfgs[0]['d']
        if freeze_at > 0:
            self._freeze_module_params(self.layer1)

        self.layer2 = self._make_layer(self.layer_cfgs[0]['d'],
                                       self.layer_cfgs[1], dprs=dprs[1],
                                       layerid=2)
        self._out_feature_strides['layer2'] = down_strides[1]
        self._out_feature_channels['layer2'] = self.layer_cfgs[1]['d']
        if freeze_at > 1:
            self._freeze_module_params(self.layer1)
        ...
        self.layer3 = self._make_layer(self.layer_cfgs[1]['d'],
                                       ...
                                       layerid=3)
        self._out_feature_strides['layer3'] = down_strides[2]
        self._out_feature_channels['layer3'] = self.layer_cfgs[2]['d']
        if freeze_at > 2:
            self._freeze_module_params(self.layer1)
        ...
        if self.num_layers == 3:
            ...
            self.layer4 = self._make_layer(self.layer_cfgs[2]['d'],
                                           ...
                                           layerid=4)
            self._out_feature_strides['layer4'] = down_strides[3]
            self._out_feature_channels['layer4'] = self.layer_cfgs[3]['d']
            if freeze_at > 3:
                self._freeze_module_params(self.layer1)            
        else:
        ...
        # Classifier head
        self.head = nn.Linear(self.out_planes, num_classes) if num_classes > 0 else nn.Identity()
        self._out_feature_strides['head'] = stride
        self._out_feature_channels['head'] = num_classes

        assert self._size_divisibility==stride, "Some stride down layer has been ignored!"

        self.apply(self._init_weights)

    def _freeze_module_params(self, module):
    ...
    def forward_w_glos(self, x):
        """
        In above adaption, we abandon the global tokens, 
        here, we study whether the global tokens are helpful
        """
        B = x.shape[0]
        outputs = {}
        x, nx, ny = self.layer1((x, None, None))
        if "layer1" in self._out_features:
            glos_avg = x[:, :self.Nglos[0]].mean(1, keepdim=True)
            outputs["layer1"] = (x[:, self.Nglos[0]:] + glos_avg).transpose(-2, -1).reshape(B, -1, nx, ny)

        x = x[:, self.Nglos[0]:].transpose(-2, -1).reshape(B, -1, nx, ny)
        x, nx, ny = self.layer2((x, nx, ny))
        if "layer2" in self._out_features:
            glos_avg = x[:, :self.Nglos[1]].mean(1, keepdim=True)
            outputs["layer2"] = (x[:, self.Nglos[1]:] + glos_avg).transpose(-2, -1).reshape(B, -1, nx, ny)

        x = x[:, self.Nglos[1]:].transpose(-2, -1).reshape(B, -1, nx, ny)
        x, nx, ny = self.layer3((x, nx, ny))
        if "layer3" in self._out_features:
            glos_avg = x[:, :self.Nglos[2]].mean(1, keepdim=True)
            outputs["layer3"] = (x[:, self.Nglos[2]:] + glos_avg).transpose(-2, -1).reshape(B, -1, nx, ny)

        if self.layer4 is not None:
            x = x[:, self.Nglos[2]:].transpose(-2, -1).reshape(B, -1, nx, ny)
            x, nx, ny = self.layer4((x, nx, ny))
            if "layer4" in self._out_features:
                glos_avg = x[:, :self.Nglos[3]].mean(1, keepdim=True)
                outputs["layer4"] = (x[:, self.Nglos[3]:] + glos_avg).transpose(-2, -1).reshape(B, -1, nx, ny)
...
def build_msvit_backbone(cfg, input_shape):
    args = dict(
    ...
    args['redraw_interval'] = cfg.MODEL.TRANSFORMER.MSVIT.REDRAW_INTERVAL

    return MsViT(**args)


@BACKBONE_REGISTRY.register()
def build_retinanet_msvit_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
    ...
    """
    bottom_up = build_msvit_backbone(cfg, input_shape)
    in_features = cfg.MODEL.FPN.IN_FEATURES
    out_channels = cfg.MODEL.FPN.OUT_CHANNELS
    in_channels_p6p7 = bottom_up.output_shape()[cfg.MODEL.TRANSFORMER.OUT_FEATURES[-1]].channels
    backbone = FPN(
        bottom_up=bottom_up,
...
@BACKBONE_REGISTRY.register()
def build_msvit_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
    Returns:
