...
__all__ = ["GeneralizedRCNNLogisticCSI"]


@META_ARCH_REGISTRY.register()
class GeneralizedRCNNLogisticCSI(nn.Module):
    """
    ...
    def forward(self, batched_inputs: Tuple[Dict[str, torch.Tensor]]):
        ...
        # breakpoint()
        rotate_degree = np.random.randint(4, size=len(batched_inputs))
        import copy
        batched_inputs_aug = copy.deepcopy(batched_inputs)
        for index in range(len(batched_inputs)):
            batched_inputs_aug[index]['image'] = torch.rot90(batched_inputs_aug[index]['image'],
                                                             rotate_degree[index], [1,2])
            if rotate_degree[index] == 1 or rotate_degree[index] == 3:
                # breakpoint()
                for index1 in range(len(batched_inputs_aug[index]['instances'].gt_boxes.tensor)):
                    # breakpoint()
                    tensor = batched_inputs_aug[index]['instances'].gt_boxes.tensor[index1]
                    new_tensor = copy.deepcopy(tensor)
                    new_tensor[0] = tensor[1]
                    new_tensor[1] = tensor[0]
                    new_tensor[2] = tensor[3]
                    new_tensor[3] = tensor[2]
                    batched_inputs_aug[index]['instances'].gt_boxes.tensor[index1] = new_tensor

                    # tensor = tensor.permute(1,0,3,2)
        # breakpoint()
        rotate_degree = torch.from_numpy(rotate_degree).cuda()
        images = self.preprocess_image(batched_inputs)
        images_rotated = self.preprocess_image(batched_inputs_aug)
        ...
        if "instances" in batched_inputs_aug[0]:
            gt_instances_aug = [x["instances"].to(self.device) for x in batched_inputs_aug]
        else:
            gt_instances_aug = None

        features_aug = self.backbone(images_rotated.tensor)



        if self.proposal_generator is not None:
            proposals, proposal_losses = self.proposal_generator(images, features, gt_instances)
            proposals_aug, proposal_losses_aug = self.proposal_generator(images_rotated, features_aug, gt_instances_aug)
        ...
        _, detector_losses = self.roi_heads(images, features, proposals,
                                            images_rotated, features_aug, proposals_aug,
                                            rotate_degree,
                                            iteration, gt_instances, gt_instances_aug)
        ...
        losses.update({'loss_rpn_cls_aug': proposal_losses_aug['loss_rpn_cls'],
                       'loss_rpn_loc_aug': proposal_losses_aug['loss_rpn_loc']})
