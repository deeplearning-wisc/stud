...
__all__ = ["GeneralizedRCNN1"]


@META_ARCH_REGISTRY.register()
class GeneralizedRCNN1(nn.Module):
    """
    ...
    def inference(
        ...
        if do_postprocess:
            ...
            return GeneralizedRCNN1._postprocess(results, batched_inputs, images.image_sizes)
        else:
            return results
    ...
    def inference_my(self,
        ...
        # ====ODIN======
        # speckle = lambda x: torch.clamp(x + x * torch.randn_like(x.float()), 0, 255)
        # # breakpoint()
        # batched_inputs[0]['image'] = speckle(batched_inputs[0]['image'])
        # ====ODIN======
        ...
        # import ipdb;
        # ipdb.set_trace()
        if self.cfg.MODEL.ROI_HEADS.NAME == 'ROIHeadsLogisticODINC':
            x_temp = self.roi_heads.norm(box_features)
            w_temp = self.roi_heads.norm(self.roi_heads.h.weight)

            aug_logits = torch.matmul(x_temp, w_temp.T)

            below = self.roi_heads.g(box_features)
            aug_logits = aug_logits  # / (below + 1e-5)
        ...
        if self.cfg.MODEL.ROI_HEADS.NAME == 'ROIHeadsLogisticODINC':
            inter_feat = aug_logits
        else:
            # for ODIN
            # inter_feat = box_cls / 1000 - torch.max(box_cls, 1, keepdim=True)[0]
            inter_feat = box_cls
