...
class SSRCNNCHEAP(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        # breakpoint()
        id_temp = torch.randperm(self.cfg.DATALOADER.PAIR_OFFSET_RANGE + 1)[:4]

        images.tensor = images.tensor[id_temp]
        images.image_sizes = images.image_sizes[id_temp]
        for key in list(features.keys()):
            features[key] = features[key][id_temp]
        proposals = proposals[id_temp]
        gt_instances = gt_instances[id_temp]
        ...
        if isinstance(detector_losses, tuple):
            ...
            for i in range(len(self.ss_head)):
                ...
                if 'build_cycle_energy_head' == self.cfg.MODEL.SS.NAME[0] or \
                        ...
                        "build_cycle_energy_direct_add_att_head" == self.cfg.MODEL.SS.NAME[0]:
                    # breakpoint()
                    ...
                    if fea_for_reg is None:
                        ...
                        losses.update({'ene_reg_loss': ene_loss})
                        del binary_labels
                elif "build_cycle_energy_direct_add_all_head" == self.cfg.MODEL.SS.NAME[0]:
                    ...
                    if fea_for_reg is None:
                        ...
                        predictions = self.roi_heads.box_predictor(fea_for_reg)
                        binary_labels = torch.ones(len(predictions[0])).cuda()
                        binary_labels[int(len(predictions[0]) / (self.cfg.DATALOADER.PAIR_OFFSET_RANGE + 1)):] = 0
