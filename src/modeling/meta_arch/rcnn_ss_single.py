...
__all__ = ["SSRCNNSingle"]


@META_ARCH_REGISTRY.register()
class SSRCNNSingle(nn.Module):
    """
            ...
            for i in range(len(self.ss_head)):
                ...
                if 'build_cycle_energy_head' == self.cfg.MODEL.SS.NAME[0] or \
                    ...
                    if fea_for_reg is None:
                        ...
                        binary_labels = torch.zeros(len(predictions[0])).cuda()
                        binary_labels[int(len(predictions[0]) / 2):] = 1
