...
class SSRCNNmixup(nn.Module):
    ...
    def mixup_data(self, x, alpha=1, use_cuda=True):
        ...
        if use_cuda:
            ...
            for index1 in range(batch_size):
                index[index1] = torch.randint(0, batch_size, (1,))[0]
    ...
    def forward(self, batched_inputs):
        ...
        images_mixup, index = self.mixup_data(images.tensor[1:])
        images.tensor = torch.cat([images.tensor[0].unsqueeze(0), images_mixup], 0)
        # breakpoint()
