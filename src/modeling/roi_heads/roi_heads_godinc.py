...
class ROIHeadsLogisticODINC(ROIHeads):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.h = nn.Linear(1024, self.num_classes + 1, bias=False)
        nn.init.kaiming_normal_(self.h.weight.data, nonlinearity="relu")
        # self.h.bias.data = torch.zeros(size=self.h.bias.size()).cuda()
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances]):
        ...
        # above = self.h(box_features)
        x_temp = self.norm(box_features)
        w_temp = self.norm(self.h.weight)

        aug_logits = torch.matmul(x_temp, w_temp.T)

        below = self.g(box_features)
        ...
        if self.training:
            ...
            losses = {
                ...
                "loss_cls_aug": cross_entropy(aug_logits, gt_classes, reduction='mean'),
