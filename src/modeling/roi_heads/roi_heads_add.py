...
from detectron2.layers import ShapeSpec, nonzero_tuple, cross_entropy,cat
...
class StandardROIHeadsSSAdd(ROIHeads):
    """
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        if self.training:
            losses, add_stuff = self._forward_box(features_list, proposals)
            ...
            return proposals, (losses, box_features), add_stuff
        else:
    ...
    def _forward_box_additional(self, box_features1, add_stuff):
        """
        ...
        box_features = torch.cat([box_features1, add_stuff[1]],0)
        box_features = self.box_head(box_features)
        predictions = self.box_predictor(box_features)
        # print(len(box_features))
        if self.training:
            # breakpoint()
            gt_classes = torch.cat([torch.ones(len(box_features1)).cuda()*(self.cfg.MODEL.ROI_HEADS.NUM_CLASSES-1), add_stuff[0]],0)
            # print(gt_classes)
            # breakpoint()
            losses = {'loss_cls_add': cross_entropy(predictions[0], gt_classes.long(), reduction="mean")}
    ...
    def _forward_box(self, features, proposals):
        ...
        if self.training:
            ...
            return losses, [gt_classes, box_features]
        else:
            pred_instances, _ = self.box_predictor.inference(
                predictions, proposals
            )
