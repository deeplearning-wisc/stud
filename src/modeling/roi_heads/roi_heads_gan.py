...
class ROIHeadsLogisticGAN(ROIHeads):
    """
    ...
    def forward_special(
        ...
        if self.training:
            losses = self._forward_box_special(features, proposals)
            # Usually the original proposals used by the box head are used by the mask, keypoint
            # heads. But when `self.train_on_pred_boxes is True`, proposals will contain boxes
            # predicted by the box head.
            # losses.update(self._forward_mask(features, proposals))
    ...
    def _forward_box_special(self, features: Dict[str, torch.Tensor], proposals: List[Instances]):
        """
        Forward logic of the box prediction branch. If `self.train_on_pred_boxes is True`,
            the function puts predicted boxes in the `proposal_boxes` field of `proposals` argument.
        Args:
        ...
        if self.training:
            ...
            if len(proposals):
                ...
                return_sth = predictions[0][selected_fg_samples]
