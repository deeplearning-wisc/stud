...
class CycleEnergyDirectAddAllRandomHead(nn.Module):
    def __init__(self, cfg, cin):
        super(CycleEnergyDirectAddAllRandomHead, self).__init__()
        ...
        if prev_boxes is not None:
            ...
            for i in range(0, len(idxs), self.cfg.DATALOADER.SELCTED_NUMBER + 1):
                ...
                if feat_u.size(0) == 0:
                    ...
                    random_indices = np.random.choice(np.arange(len(soft_target_score[0])),
                                                      len(soft_target_score), replace=True)
                    random_indices = torch.from_numpy(random_indices).to(v_all.device)



                    if neg_fea is None:
                        neg_fea = self.add(v_all).view(-1, 256*49)[random_indices]+\
                            torch.matmul(soft_target_score, self.add(v_all).view(-1, 256 * 49)) / 10000
                    else:
                        neg_fea = torch.cat(
                            [neg_fea, self.add(v_all).view(-1, 256*49)[random_indices]+\
                             torch.matmul(soft_target_score, self.add(v_all).view(-1, 256 * 49)) / 10000], 0)
                    # breakpoint()
                # assert frame ==  - 1
...
def build_cycle_energy_direct_add_all_random_head(cfg, input_shape):
    in_channels = cfg.MODEL.FPN.OUT_CHANNELS
    rot_head = CycleEnergyDirectAddAllRandomHead(cfg, in_channels)
    return rot_head
