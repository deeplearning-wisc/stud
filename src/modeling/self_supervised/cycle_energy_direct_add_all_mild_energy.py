...
class CycleEnergyDirectAddAllMildHead(nn.Module):
    def __init__(self, cfg, cin):
        super(CycleEnergyDirectAddAllMildHead, self).__init__()
        ...
        if prev_boxes is not None:
            ...
            for i in range(0, len(idxs), self.cfg.DATALOADER.SELCTED_NUMBER + 1):
                ...
                if u.size(0) == 0:
                    ...
                    if neg_fea is None:
                        neg_fea = self.add(v_all).view(-1, 256 * 49)
                    else:
                        neg_fea = torch.cat(
                            [neg_fea, self.add(v_all).view(-1, 256 * 49)], 0)
                    # breakpoint()
                # assert frame ==  - 1
...
def build_cycle_energy_direct_add_all_mild_head(cfg, input_shape):
    in_channels = cfg.MODEL.FPN.OUT_CHANNELS
    rot_head = CycleEnergyDirectAddAllMildHead(cfg, in_channels)
    return rot_head
