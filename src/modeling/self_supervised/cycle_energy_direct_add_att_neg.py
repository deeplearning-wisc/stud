...
class CycleEnergyDirectAddAttNegHead(nn.Module):
    def __init__(self, cfg, cin):
        super(CycleEnergyDirectAddAttNegHead, self).__init__()
        ...
        if prev_boxes is not None:
            ...
            for i in range(0, len(idxs), self.cfg.DATALOADER.PAIR_OFFSET_RANGE + 1):
                ...
                if feat_u.size(0) == 0:
                    ...
                    if neg_fea is None:
                        # breakpoint()
                        # neg_fea_trans = self.enc2(neg_fea_temp).view(
                        #                   self.cfg.DATALOADER.PAIR_OFFSET_RANGE,
                        #                   pos_fea_cur.size(0),
                        #                    -1).permute(1,0,2)
                        # # breakpoint()
                        # pos_fea_trans = self.enc2(pos_fea_cur).view(pos_fea_cur.size(0),256, 1)
...
def build_cycle_energy_direct_add_att_neg_head(cfg, input_shape):
    in_channels = cfg.MODEL.FPN.OUT_CHANNELS
    rot_head = CycleEnergyDirectAddAttNegHead(cfg, in_channels)
    return rot_head
