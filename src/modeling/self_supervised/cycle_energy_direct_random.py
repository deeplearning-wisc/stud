...
class CycleEnergyDirectRandomHead(nn.Module):
    def __init__(self, cfg, cin):
        super(CycleEnergyDirectRandomHead, self).__init__()
    ...
    def computer_corr_softmax(self, feat_u, feat_v):
        ...
        count = len(back_dist)
        return loss, correct, count, sim_score
        ...
        if prev_boxes is not None:
            ...
            for i in range(0, len(idxs), 2):
                ...
                if feat_u.size(0) == 0:
                    ...
                    random_indices = torch.from_numpy(random_indices).to(v.device)

                    # breakpoint()
                    if pos_fea is None:
                        pos_fea = self.add(u).view(-1, 256*49)
                        neg_fea = self.add(v).view(-1, 256*49)[random_indices]
                        #torch.matmul(soft_target_score, v.view(-1, 256*49))
                    else:
                        pos_fea = torch.cat([pos_fea, self.add(u).view(-1, 256*49)], 0)
                        neg_fea = torch.cat([neg_fea, self.add(v).view(-1, 256*49)[random_indices]], 0)
...
def build_cycle_energy_direct_add_random_head(cfg, input_shape):
    in_channels = cfg.MODEL.FPN.OUT_CHANNELS
    rot_head = CycleEnergyDirectRandomHead(cfg, in_channels)
    return rot_head
