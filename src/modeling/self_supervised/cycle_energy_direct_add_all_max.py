...
class CycleEnergyDirectAddAllMaxHead(nn.Module):
    def __init__(self, cfg, cin):
        super(CycleEnergyDirectAddAllMaxHead, self).__init__()
        ...
        if prev_boxes is not None:
            ...
            for i in range(0, len(idxs), self.cfg.DATALOADER.SELCTED_NUMBER + 1):
                ...
                if feat_u.size(0) == 0:
                    ...
                    if neg_fea is None:
                        # a trick here to make the enc1 trainable.
                        neg_fea = self.add(v_all).view(-1, 256*49)[max_indices]+torch.matmul(soft_target_score, self.add(v_all).view(-1, 256 * 49))/10000
                        # breakpoint()
                    else:
                        neg_fea = torch.cat(
                            [neg_fea, self.add(v_all).view(-1, 256*49)[max_indices]+\
...
def build_cycle_energy_direct_add_all_max_head(cfg, input_shape):
    in_channels = cfg.MODEL.FPN.OUT_CHANNELS
    rot_head = CycleEnergyDirectAddAllMaxHead(cfg, in_channels)
    return rot_head
