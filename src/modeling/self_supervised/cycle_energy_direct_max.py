...
class CycleEnergyDirectMaxHead(nn.Module):
    def __init__(self, cfg, cin):
        super(CycleEnergyDirectMaxHead, self).__init__()
            ...
            for i in range(0, len(idxs), 2):
                ...
                if feat_u.size(0) == 0:
                    ...
                    if pos_fea is None:
                        ...
                        neg_fea = self.add(v).view(-1, 256*49)[max_indices]
                        #torch.matmul(soft_target_score, v.view(-1, 256*49))
                    else:
                        pos_fea = torch.cat([pos_fea, self.add(u).view(-1, 256*49)], 0)
                        neg_fea = torch.cat([neg_fea, self.add(v).view(-1, 256*49)[max_indices]], 0)
...
def build_cycle_energy_direct_add_max_head(cfg, input_shape):
    in_channels = cfg.MODEL.FPN.OUT_CHANNELS
    rot_head = CycleEnergyDirectMaxHead(cfg, in_channels)
    return rot_head
