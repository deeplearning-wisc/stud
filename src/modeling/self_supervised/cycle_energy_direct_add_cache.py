    ...
    def forward(self, features, prev_boxes=None):
        ...
        if prev_boxes is not None:
            ...
            for i in range(0, len(idxs), 2):
                ...
                # breakpoint()
                if self.pos_length >= 1000 and self.neg_length >= 1000:
                    import numpy as np
                    np.save('bdd_pos_single_frame_1000.npy', self.saved_pos)
                    np.save('bdd_neg_single_frame_1000.npy', self.saved_neg)
                    breakpoint()
                else:
                    self.saved_neg.append(neg_fea.cpu().data.numpy())
                    self.saved_pos.append(pos_fea.cpu().data.numpy())
                    self.pos_length += len(pos_fea)
                    self.neg_length += len(neg_fea)
                total_loss += loss*cnt
                corrects += correct
                counts += cnt
            # breakpoint()
