        ...
        if prev_boxes is not None:
            ...
            for i in range(0, len(idxs), 2):
                ...
                if feat_u.size(0) == 0:
                    ...
                    loss, correct, cnt, soft_target_score, dist = self.computer_corr_softmax(
                        feat_u, feat_v)
                    # max_indices = torch.argmax(soft_target_score, 1).view(-1)
                    # print(soft_target_score.shape)
                    # breakpoint()
                    # temp
                    # fea_temp = roi_head.box_head(torch.matmul(soft_target_score, self.add(v).view(-1, 256*49)).view(-1,256,7,7))
                    # predictions1 = roi_head.box_predictor(fea_temp)
                    # energy_scores_all = torch.logsumexp(predictions1[0][:, :-1], dim=1)
                    # print(energy_scores_all)
                    # fea_temp = roi_head.box_head(self.add(u))
                    # predictions1 = roi_head.box_predictor(fea_temp)
                    ...
                    if pos_fea is None:
                        ...
                        neg_fea = torch.cat([neg_fea, torch.matmul(
                            soft_target_score, self.add(v).view(-1, 256*49))], 0)
